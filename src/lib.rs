...
impl BF {
    ...
    pub fn set_code(&mut self, c: String) {
        ...
        c.into_bytes().iter().for_each(|char| match char {
        ...
        if loop_error {
            ...
            log(&format!(
                "Writing code: \n\n{}.\n\nto program memory",
                from_arr_to_ascii(&self.code)
            ));
        } else {
            err(&format!(
                "Could not compile brainfuck, found {} unmatched loops",
                loop_count
            ));
            self.code = vec![];
        }
    ...
    pub fn get_state(&self) -> String {
        format!(
            "{{\n \"program_counter\": {},\n \"memory\": {:?},\n \"memory_counter\": {}}}",
            self.program_counter, self.memory, self.memory_counter,
        )
    }
    ...
    pub fn step(&mut self) -> bool {
        if self.code.len() == self.program_counter as usize {
            ...
            match self.code[self.program_counter as usize] {
                ...
                b'.' => {
                    ...
                    if self.memory[(self.memory_counter as usize) % 30000].is_ascii() {
                        el.set_inner_html(&format!(
                            ...
                            text,
                            std::str::from_utf8(&[
                                self.memory[(self.memory_counter as usize) % 30000]
                            ])
                            .unwrap()
                        ));
                    } else {
                ...
                _ => {
                    log(&format!(
                        "Could not Interpret character at index: {}",
                        self.memory_counter % 30000
                    ));
                }
            }
            true
